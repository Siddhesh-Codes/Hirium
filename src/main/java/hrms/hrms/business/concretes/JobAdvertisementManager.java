...
public class JobAdvertisementManager implements JobAdvertisementService {
	...
	public DataResult<JobAdvertisementDto> getById(Integer id) {
		var adOpt = jobAdvertisementDao.findById(id);
		if (adOpt.isPresent()) {
			return new SuccessDataResult<>(toDto(adOpt.get()), "Job advertisement found.");
		}
		return new ErrorDataResult<>("Job advertisement not found with id: " + id);
	}

	private JobAdvertisementDto toDto(JobAdvertisement j) {
		JobAdvertisementDto dto = new JobAdvertisementDto();
		dto.setId(j.getId());
		dto.setJobTitle(j.getJobPosition() != null ? j.getJobPosition().getTitle() : null);
		dto.setCompanyName(j.getEmployer() != null ? j.getEmployer().getCompanyName() : null);
		dto.setCity(j.getCity() != null ? j.getCity().getCityName() : null);
		dto.setOpenPositionCount(j.getOpenPositionCount());
		dto.setMinSalary(j.getMinSalary());
		dto.setMaxSalary(j.getMaxSalary());
		dto.setReleaseDate(j.getReleaseDate());
		...
		dto.setCityId(j.getCity() != null ? j.getCity().getId() : null);
		dto.setJobPositionId(j.getJobPosition() != null ? j.getJobPosition().getId() : null);
		dto.setCompanyWebPage(j.getEmployer() != null ? j.getEmployer().getCompanyWebPage() : null);
		dto.setCompanyEmail(j.getEmployer() != null ? j.getEmployer().getEmail() : null);
		dto.setCompanyPhoneNumber(j.getEmployer() != null ? j.getEmployer().getPhoneNumber() : null);
		return dto;
	}
}
